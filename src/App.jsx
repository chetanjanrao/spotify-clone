...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (_token) {
      ...
      const setupSpotifySession = async (token) => {
        ...
        // Example: Fetch a featured playlist to display something on the home page
        const featuredPlaylists = await spotify.getFeaturedPlaylists({ limit: 1 });
        const playlistId = featuredPlaylists.playlists.items[0]?.id;
        if (playlistId) {
          const discoverWeekly = await spotify.getPlaylist(playlistId);
          dispatch({ type: 'SET_DISCOVER_WEEKLY', discover_weekly: discoverWeekly });
        }
      };

      setupSpotifySession(_token).catch(console.error);
    }
  }, []);
  return <>{token ? <HomePage spotify={spotify} /> : <Login />}</>;
}
